...
import org.hypothesis.resource.context.HierarchicalMessageSource;
import org.hypothesis.resource.context.MessageSource;
import org.hypothesis.resource.context.MessageSourceResolvable;
import org.hypothesis.resource.context.NoSuchMessageException;

/**
 * Abstract implementation of the {@link HierarchicalMessageSource} interface,
 * implementing common handling of message variants, making it easy to implement
 * a specific strategy for a concrete MessageSource.
 *
 * <p>
 * Subclasses must implement the abstract {@link #resolveCode} method. For
 * efficient resolution of messages without arguments, the
 * {@link #resolveCodeWithoutArguments} method should be overridden as well,
 * resolving messages without a MessageFormat being involved.
 *
 * <p>
 * <b>Note:</b> By default, message texts are only parsed through MessageFormat
 * if arguments have been passed in for the message. In case of no arguments,
 * message texts will be returned as-is. As a consequence, you should only use
 * MessageFormat escaping for messages with actual arguments, and keep all other
 * messages unescaped. If you prefer to escape all messages, set the
 * "alwaysUseMessageFormat" flag to "true".
 *
 * <p>
 * Supports not only MessageSourceResolvables as primary messages but also
 * resolution of message arguments that are in turn MessageSourceResolvables
 * themselves.
 *
 * <p>
 * This class does not implement caching of messages per code, thus subclasses
 * can dynamically change messages over time. Subclasses are encouraged to cache
 * their messages in a modification-aware fashion, allowing for hot deployment
 * of updated messages.
 *
...
public abstract class AbstractMessageSource extends MessageSourceSupport implements HierarchicalMessageSource {
	...
	/**
	 * Return a Properties object defining locale-independent common messages,
	 * if any.
	 */
	 ...
	 * returning {@code null} if not found. Does <i>not</i> fall back to the
	 * code as default message. Invoked by {@code getMessage} methods.
	 * 
	 * @param code
	 ...
	 * will usually receive a NoSuchMessageException from {@code getMessage}.
	 * 
	 * @param code
	 ...
	 * <p>
	 * Unfortunately, {@code java.text.MessageFormat} is not implemented in an
	 * efficient fashion. In particular, it does not detect that a message
