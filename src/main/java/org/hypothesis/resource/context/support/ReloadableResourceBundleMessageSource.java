...
package org.hypothesis.resource.context.support;
...
import org.hypothesis.resource.context.ResourceLoaderAware;
import org.hypothesis.resource.core.io.DefaultResourceLoader;
import org.hypothesis.resource.core.io.Resource;
import org.hypothesis.resource.core.io.ResourceLoader;
import org.hypothesis.resource.util.DefaultPropertiesPersister;
import org.hypothesis.resource.util.PropertiesPersister;
 ...
 * {@link org.hypothesis.util.springframework.util.PropertiesPersister} strategy
 * from Spring {@link Resource} handles. This strategy is not only capable of
 * reloading files based on timestamp changes, but also of loading properties
 * files with a specific character encoding. It will detect XML property files
 * as well.
 *
 * <p>
 * In contrast to {@link ResourceBundleMessageSource}, this class supports
 * reloading of properties files through the {@link #setCacheSeconds
 * "cacheSeconds"} setting, and also through programmatically clearing the
 * properties cache. Since application servers typically cache all files loaded
 * from the classpath, it is necessary to store resources somewhere else (for
 * example, in the "WEB-INF" directory of a web app). Otherwise changes of files
 * in the classpath will <i>not</i> be reflected in the application.
 *
 * <p>
 ...
 * {@link ResourceBundleMessageSource}, which builds on
 * {@link java.util.ResourceBundle} - in the default mode, i.e. when caching
 * forever. With "cacheSeconds" set to 1, message lookup takes about twice as
 * long - with the benefit that changes in individual properties files are
 * detected with a maximum delay of 1 second. Higher "cacheSeconds" values
 * usually <i>do not</i> make a significant difference.
 *
 * <p>
 * This MessageSource can easily be used outside of an
 * {@link org.springframework.context.ApplicationContext}: It will use a
 * {@link org.hypothesis.core.io.springframework.core.io.DefaultResourceLoader}
 * as default, simply getting overridden with the ApplicationContext's resource
 * loader if running in a context. It does not have any other specific
 * dependencies.
 *
 * <p>
 ...
 * @see org.hypothesis.util.springframework.util.DefaultPropertiesPersister
 * @see org.hypothesis.core.io.springframework.core.io.DefaultResourceLoader
...
public class ReloadableResourceBundleMessageSource extends AbstractMessageSource implements ResourceLoaderAware {
	...
	public void setBasenames(String... basenames) {
		if (basenames != null) {
			...
			for (int i = 0; i < basenames.length; i++) {
				...
				Validate.notBlank(basename, "Basename must not be empty");
	 ...
	 *            the default charset
	 * @see #setFileEncodings
	 * @see org.hypothesis.util.springframework.util.PropertiesPersister#load
	 */
	 ...
	 * an application server environment, where the system Locale is not
		...
		if (this.fallbackToSystemLocale && !locale.equals(Locale.getDefault())) {
			...
			for (String fallbackFilename : fallbackFilenames) {
				if (!filenames.contains(fallbackFilename)) {
					// Entry for fallback locale that isn't already in filenames
					// list.
	 ...
	 * can be {@code null} if not cached before, or a timed-out cache entry
		...
		if (resource.exists()) {
			...
			if (this.cacheMillis >= 0) {
				...
				try {
					...
					if (logger.isDebugEnabled()) {
						logger.debug(
								resource + " could not be resolved in the file system - assuming that it hasn't changed",
								ex);
					}
					fileTimestamp = -1;
				}
	...
	public String toString() {
		return getClass().getName() + ": basenames=[" + Arrays.stream(basenames).collect(Collectors.joining(",")) + "]";
	}
